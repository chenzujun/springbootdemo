package com.raine.springboot.demo.controller.user;
...
public class UserController {
	...
	public PageInfo getUsers(@RequestParam("page") Integer page,
							 @RequestParam("limit") Integer limit, UserQuery userSearch) {
		logger.debug("分页查询用户列表！搜索条件：userSearch：" + userSearch + ",page:" + page
				+ ",每页记录数量limit:" + limit);
		PageInfo pdr = new PageInfo();
		try {
			if (null == page) {
				page = 1;
			}
			...
			// 获取用户和角色列表
			pdr = userService.pageUser(userSearch);
...
//			}
	...
	@ResponseBody
	public ResponseResult sendMsg(UserVo user) {
		...
		try {
			...
			String msg = "ok";
			if (msg != "ok") {
				responseResult.setCode(EnumSystemStatus.ERROR
						.getCode());
...
//				responseResult.setCode(EnumSystemStatus.PARAM_ERROR
//						.getCode());
...
//			}
			...
			User existUser = (User) SecurityUtils.getSubject().getPrincipal();
			if (null == existUser) {
				responseResult.setCode(EnumSystemStatus.NO_LOGIN.getCode());
...
//			}
			...
			User existUser = (User) SecurityUtils.getSubject().getPrincipal();
			if (null == existUser) {
				responseResult.setCode(EnumSystemStatus.NO_LOGIN
						.getCode());
				responseResult.setMessage("您未登录或登录超时，请您重新登录后再试");
				logger.debug("修改密码，结果=responseResult:" + responseResult);
				return responseResult;
